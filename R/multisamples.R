...
pruning <- function(td, trait_names, sigma2, node_index) {
  ...
  if (descendants[1] <= ntaxa) { # if left descendant is a tip
    edge_left <- phy$edge.length[phy$edge[,2] == descendants[1]] + epsilon
    # this epsilon is actually variance not the branch length
    # need correction here
    chr_left <- characters[descendants[1],]
    loglik_left <- 0 # values from the data
  }
  ...
  if (descendants[2] <= ntaxa) { # if right offspring node is a tip
    edge_right <- phy$edge.length[phy$edge[,2] == descendants[2]] + epsilon
    # same for this epsilon
    chr_right <- characters[descendants[2],]
    loglik_right <- 0 # values from the data
  }
